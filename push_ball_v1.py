...
class Humanoid(PipelineEnv):
    def __init__(
        ...
        **kwargs,
        ...
        mj_model = mujoco.MjModel.from_xml_path(
            (HUMANOID_ROOT_PATH / "humanoid_with_ball_pendulum.xml").as_posix()
        )
    ...
    def get_hand_touch_obs(self, data: mjx.Data) -> jp.ndarray:
        """
        Returns binary indicators (0 or 1) if left or right hand is touching the ball.
        """
        ...
        ball_geom_id = self.ball_geom_id
        hand_right_geom_id = self.hand_right_geom_id
        hand_left_geom_id = self.hand_left_geom_id
    
        # Check for right-hand ball contacts
        right_hand_contact = jp.any(
            jp.logical_or(
                jp.logical_and(geom1_ids == ball_geom_id, geom2_ids == hand_right_geom_id),
                jp.logical_and(geom2_ids == ball_geom_id, geom1_ids == hand_right_geom_id),
            )
        )
    
        # Check for left-hand ball contacts
        left_hand_contact = jp.any(
            jp.logical_or(
                jp.logical_and(geom1_ids == ball_geom_id, geom2_ids == hand_left_geom_id),
                jp.logical_and(geom2_ids == ball_geom_id, geom1_ids == hand_left_geom_id),
            )
        )

        # Return binary indicators as floats (0.0 or 1.0)
        return jp.array([right_hand_contact, left_hand_contact], dtype=jp.float32)


    def reward_hand_contact(self, state):
        """
        Compute reward for maintaining contact between the humanoid's hand and the ball.
        ...
        # Compute boolean arrays indicating matches with ball and hands
        ball_geom_id = self.ball_geom_id
        hand_geom_ids = jp.array([self.hand_right_geom_id, self.hand_left_geom_id])
    
        # Check for each contact pair if it matches ball-hand pair
        is_ball_hand_contact = jp.logical_or(
            jp.logical_and(
                geom1_ids == ball_geom_id,
                jp.any(geom2_ids[:, None] == hand_geom_ids, axis=1)
            ),
            jp.logical_and(
                geom2_ids == ball_geom_id,
                jp.any(geom1_ids[:, None] == hand_geom_ids, axis=1)
            )
        )
    
        # If any contact is ball-hand contact, reward = 1, else 0
        reward = jp.where(jp.any(is_ball_hand_contact), 1.0, 0.0)
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        reward = healthy_reward - ctrl_cost + self._hand_contact_weight * self.reward_hand_contact(state)
        done = 1.0 - is_healthy if self._terminate_when_unhealthy else 0.0
        
        state.metrics.update(reward_quadctrl=-ctrl_cost, reward_alive=healthy_reward)
    ...
    def _get_obs(self, data: mjx.Data, action: jp.ndarray) -> jp.ndarray:
        ...
        # Add hand touch observations
        hand_touch_obs = self.get_hand_touch_obs(data)
        obs_list.append(hand_touch_obs)
